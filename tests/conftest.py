"""What every test may share: the data directory, the pUC19 and GFP records, and one plan of them.
...
@pytest.fixture(scope="session")
def puc19(data_dir: Path) -> SequenceRecord:
    """The pUC19 vector, circular, with its multiple cloning site annotated."""
    from liulab_mbio.io import read_record

    return read_record(data_dir / "pUC19.dna")


@pytest.fixture(scope="session")
def gfp(data_dir: Path) -> SequenceRecord:
    """The GFP coding sequence, linear."""
    from liulab_mbio.io import read_record

    return read_record(data_dir / "GFP.dna")
...
def plan(puc19: SequenceRecord, gfp: SequenceRecord) -> Plan:
    ...
    Built once per session. A plan and its records are frozen, so a test wanting another builds
    it, with `dataclasses.replace` or `plan_assembly`.
    """
