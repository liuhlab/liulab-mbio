...
`labels` keeps labels apart, on bare boxes; `fonts` measures text in the pinned faces; `svg` holds
the shapes and writes them; `page` wraps the SVG in one offline HTML page; `convert` turns it into
a PNG or a PDF. A drawing is laid out as geometry before anything draws, per
`docs/adr/0006-drawings-as-geometry.md`.
