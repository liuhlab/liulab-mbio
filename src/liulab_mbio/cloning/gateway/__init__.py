...
PCR that puts it on an insert; `recombination` simulates one reaction on two records; `bench`
is each reaction's own components, incubation and stop; `steps` is the protocol's steps and
their order. What any pipeline shares is `liulab_mbio.bench`, and `liulab_mbio.sites` is enzyme
cut sites, which have nothing to do with an att site.
"""
