...
| plot | `plot/` | a record drawn as a map: `layers` resolves items, `circular` and `linear` lay them out, `labels` keeps labels apart, `fonts` measures, `svg` and `page` write, `convert` makes a PNG or PDF |
